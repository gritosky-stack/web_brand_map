...
/// Скачивание и распаковка наборов тайлов, лежащих в R2.
///
/// Наборов два и устроены они одинаково: в R2 лежит один файл MBTiles — обычная
/// база SQLite, — приложение качает его, раскладывает в дерево `{z}/{x}/{y}`
...
///
/// Отличий между наборами всего два, и оба спрятаны в `TileSetSpec`:
/// расширение тайла (`pbf` у векторной топоосновы, `jpg` у растровой
/// исторической карты) и объём. Растровые тайлы не gzip-нуты, но проверять
/// это отдельно не нужно: `gunzip` возвращает nil на негзипованных данных,
/// и запись идёт как есть.
///
...
/// сама и будит приложение по готовности. У каждого набора сессия своя,
/// иначе система не разберёт, чью загрузку она возобновляет.
///
...
struct TileSetSpec {
    let id: String
    let version: String
    /// Имя файла в R2 и на диске во время загрузки
    let archive: String
    let rootURL: URL
    /// Расширение файла тайла в разложенном дереве
    let ext: String
    ...
    /// Идентификатор фоновой сессии. У топоосновы он исторический: по нему
    /// система возвращает загрузку, начатую прошлой версией приложения, —
    /// переименуй, и незавершённая закачка у пользователя осиротеет.
    let sessionIdentifier: String

    static let base = "https://pub-46dba1bca6754d2499a2a5aa9d5c879f.r2.dev"

    var remoteURL: URL { URL(string: "\(Self.base)/maps/\(archive)")! }

    static func root(_ name: String) -> URL {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent(name, isDirectory: true)
    }

    static let topo = TileSetSpec(
        id: "topo", version: TopoTiles.version, archive: "serbia-topo-\(TopoTiles.version).mbtiles",
        rootURL: TopoTiles.rootURL, ext: "pbf", requiredFreeBytes: 2_000_000_000,
        sessionIdentifier: "wild.totskii.topo-tiles")

    static let histmap = TileSetSpec(
        id: "histmap", version: HistMapTiles.version,
        archive: "serbia-histmap-\(HistMapTiles.version).mbtiles",
        rootURL: HistMapTiles.rootURL, ext: "jpg", requiredFreeBytes: 1_500_000_000,
        sessionIdentifier: "wild.totskii.histmap-tiles")
}

final class TileSetDownloader: NSObject, ObservableObject {

    /// Топооснова: векторные тайлы Сербии и Косова
    static let topo = TileSetDownloader(spec: .topo)
    /// Историческая карта 1:75 000, растр
    static let histmap = TileSetDownloader(spec: .histmap)
    static var all: [TileSetDownloader] { [topo, histmap] }
    ...
    private var pendingArchive: URL {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent(spec.archive)
    }

    private init(spec: TileSetSpec) {
        self.spec = spec
        self.isReady = TileSet.isAvailable(spec)
        super.init()
    ...
    func start() {
        ...
        guard freeDiskBytes() > spec.requiredFreeBytes else {
            let need = ByteCountFormatter.string(fromByteCount: spec.requiredFreeBytes, countStyle: .file)
        ...
        if !TileSet.isAvailable(spec) {
            try? FileManager.default.removeItem(at: spec.rootURL)
        }

        stage = .downloading(progress: 0, bytes: 0, total: 0)
        keepScreenAwake(true)
        session.downloadTask(with: spec.remoteURL).resume()
    }
    ...
    func delete() {
        ...
        let root = spec.rootURL
        let trash = root.deletingLastPathComponent()
            .appendingPathComponent("\(spec.id)-tiles-trash-\(UUID().uuidString)")
    ...
    private func unpack(_ mbtiles: URL) {
        ...
        while sqlite3_step(stmt) == SQLITE_ROW {
            ...
            let file = root.appendingPathComponent("\(dirKey)/\(y).\(spec.ext)")
        ...
        let manifest: [String: Any] = ["version": spec.version, "tiles": written, "bytes": bytesWritten]
...
extension TileSetDownloader: URLSessionDownloadDelegate {
